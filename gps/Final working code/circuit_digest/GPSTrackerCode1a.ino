/*  Circuit Disgest Code Working oK 30 March 2025
...
// WiFi credentials
const char* ssid = "Yogeshram R";      // WiFi SSID - Replace your own
const char* password = "yogesh/1939";  // WiFi Password - Replace your own

// Server details for sending GPS data
const char* serverUrl = "https://www.circuitdigest.cloud/geolinker";  // Server URL
const char* apiKey = "54NY38PyPTeh";                                  //12 character API key for authentication
...
void setup() {
  Serial.begin(115200);
  gpsSerial.begin(115200, SERIAL_8N1, RXPin, TXPin);  // Initialize GPS module
  ...
  delay(2000);  // Ensure GPS is off
    ...
    //sendATCommand("AT+QURCCFG=\"urcport\",\"uart1\"");
   // sendATCommand("AT+QGPSCFG=\"outport\",\"uart1\""); // AT+QGPSCFG="outport","uart1"
   // delay(2000);  // Ensure GPS is off
...
//----------------------------------------- enable GPS Module
   //Serial.println("Read Serial Data");
         ...
         gpsSerial.println("AT+QGPSGNMEA=\"GGA\"");  // AT+QGPSCFG=\"nmeasrc\",1
         delay(2000);  // Short wait for a response
      //   String response = readATResponse();
         gpsSerial.println("AT+QGPSGNMEA=\"RMC\"");
         delay(2000);  // Short wait for a response
      //   String response = readATResponse();
...
void processGPSData(String raw) {
  ...
  if (raw.startsWith("+QGPSGNMEA: $GNGGA")) {  // GPGGA contains location and satellite data
    parseGPGGA(raw);
    convertAndPrintLocalDateTime();       // Convert UTC to local time
  } else if (raw.startsWith("+QGPSGNMEA: $GNRMC")) {  // GPRMC contains date and speed info
    parseGPRMC(raw);
  }
}
...
void sendATCommand(const String& command) {
  ...
  while (gpsSerial.available()) {
    String response = gpsSerial.readString();
